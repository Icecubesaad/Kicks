  ...
  useEffect(() => {
    (async () => {
      ...
      if (latitude >= 49.0 && latitude <= 61.0 && longitude >= -8.0 && longitude <= 2.0) {
        setRegion('GBP');
        setCurrencyIcon('gbp');
      } else if (latitude >= 42.0 && latitude <= 83.0 && longitude >= -141.0 && longitude <= -52.0) {
        setRegion('CAD');
        setCurrencyIcon('dollar');
      } else if (latitude >= 24.0 && latitude <= 46.0 && longitude >= 123.0 && longitude <= 146.0) {
        setRegion('JPY');
        setCurrencyIcon('jpy');
      } else if (latitude >= 24.396308 && latitude <= 49.384358 && longitude >= -125.0 && longitude <= -66.93457) {
        setRegion('USA');
        setCurrencyIcon('dollar');
      } else if (latitude >= 35.0 && latitude <= 70.0 && longitude >= -10.0 && longitude <= 40.0) {
        setRegion('EUR');
        setCurrencyIcon('eur');
      } else if (latitude >= -55.0 && latitude <= -10.0 && longitude >= 113.0 && longitude <= 154.0) {
        setRegion('AUD');
        setCurrencyIcon('dollar');
      } else {
        setRegion('Other');
  ...
  return (
    <View style={styles.parent}>
      ...
      <View style={styles.optionsContainer}>
        <TouchableOpacity onPress={() => handleCompanyChange('Nike')} style={styles.option}>
          <Text style={styles.optionText}>Nike</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => handleCompanyChange('adidas')} style={styles.option}>
          <Text style={styles.optionText}>Adidas</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => handleCompanyChange('puma')} style={styles.option}>
